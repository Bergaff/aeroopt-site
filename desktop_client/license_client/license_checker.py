...
class LicenseChecker:
    ...
    def _api(self, path: str, payload: dict) -> Optional[dict]:
        ...
        try:
            ...
            try:
                ...
                pass
            print(f"[LicenseChecker] HTTP {e.code} от {url}")
        except (URLError, socket.timeout, TimeoutError, OSError) as e:
            print(f"[LicenseChecker] Сеть недоступна ({url}): {e}")
        except Exception as e:
            print(f"[LicenseChecker] Ошибка запроса {url}: {e}")
        return None
    ...
    def _save_state(self):
        ...
        try:
            ...
            print(f"[LicenseChecker] Не удалось сохранить кэш: {e}")
    ...
    def activate(self, key: str) -> Tuple[bool, str]:
        ...
        print(f"[LicenseChecker] activate() key={self.license_key} "
    ...
    def deactivate(self) -> Tuple[bool, str]:
        ...
        if self.license_key:
            ...
            if result["ok"]:
                print("[LicenseChecker] Сервер подтвердил отвязку HWID")
            elif result.get("network"):
                print("[LicenseChecker] Сеть недоступна — снимаем только локальную привязку")
        self._reset_state()
